...
const problemaSelecionadoUrl = 'https://66550ead3c1d3b6029382097.mockapi.io/selecionados';
...
async function verificarProblemaSelecionado() {
    ...
    try {
        ...
        const problemasSelecionados = dados.filter(entry => {
            console.log(`Verificando problema: ID do Aluno: ${entry.idAluno}, Selecionado: ${entry.selecionado}`);
            return entry.idAluno === alunoId && entry.selecionado === true; selecionados
        });
...
async function alunoEntregouProjeto() {
    ...
    try {
        ...
        const alunoProblemaSelecionado = problemasSelecionados.find(entry => entry.idAluno === alunoId && entry.selecionado === true);
        ...
        if (alunoProblemaSelecionado) {
            ...
            const updateResponse = await fetch(`${problemaSelecionadoUrl}/${alunoProblemaSelecionado.id}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json'
                },
...
async function atualizarProblemaRecusado() {
    ...
    try {
        ...
        const problemaAluno = problemasSelecionados.find(entry => entry.idAluno === alunoId && entry.selecionado === true);
        ...
        if (problemaAluno) {
            ...
            const updateResponse = await fetch(`${problemaSelecionadoUrl}/${problemaAluno.id}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json'
                },
